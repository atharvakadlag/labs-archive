...
double dot(vector<double>& vec1, vector<double>& vec2)
{
    ...
    double total = 0;
	// #pragma omp parallel for shared(vec1, vec2, total)
...
private:
    ...
    void train_epoch()
    {
        #pragma omp parallel for shared(traindata)
...
int main()
{
    ...
    for (int i = 0; i < n_samples; i++)
    {
        ...
        vector<double> coords = {};
        for(int i = 0; i < 50; i++) {
            coords.push_back(float(rand())/float((RAND_MAX))*9999.0);
        }
        bool is_type_A;
        is_type_A = rand() > 1 ? false : true;
        traindata.push_back(Sample(coords, is_type_A));
    }
    double lr;
